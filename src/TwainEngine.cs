...
public class TwainEngine : IDisposable
{
    ...
    public struct TW_VERSION { public ushort MajorNum; public ushort MinorNum; public ushort Language; public ushort Country; [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 34)] public string Info; }

    [StructLayout(LayoutKind.Sequential, Pack = 2)]
    public struct TW_IDENTITY { public uint Id; public TW_VERSION Version; public ushort ProtocolMajor; public ushort ProtocolMinor; public uint SupportedGroups; [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 34)] public string Manufacturer; [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 34)] public string ProductFamily; [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 34)] public string ProductName; }

    [StructLayout(LayoutKind.Sequential, Pack = 2)]
    public struct TW_USERINTERFACE { public ushort ShowUI; public ushort ModalUI; public IntPtr ParentHand; }

    [StructLayout(LayoutKind.Sequential, Pack = 2)]
    public struct TW_CAPABILITY { public ushort Cap; public ushort ConType; public IntPtr hContainer; }

    [StructLayout(LayoutKind.Sequential, Pack = 2)]
    public struct TW_ONEVALUE { public ushort ItemType; public uint Item; }
    ...
    [DllImport("kernel32.dll", EntryPoint = "GlobalAlloc")] private static extern IntPtr GlobalAlloc(uint uFlags, UIntPtr dwBytes);
    [DllImport("kernel32.dll", EntryPoint = "GlobalLock")] private static extern IntPtr GlobalLock(IntPtr hMem);
    [DllImport("kernel32.dll", EntryPoint = "GlobalUnlock")] private static extern bool GlobalUnlock(IntPtr hMem);
    [DllImport("kernel32.dll", EntryPoint = "GlobalFree")] private static extern IntPtr GlobalFree(IntPtr hMem);
    ...
    public TwainEngine()
    {
        ...
        _appIdentity = new TW_IDENTITY { Id = 0, Version = new TW_VERSION { MajorNum = 1, MinorNum = 0, Language = 19, Country = 1, Info = "V1" }, ProtocolMajor = 2, ProtocolMinor = 3, SupportedGroups = 0x00000003, Manufacturer = "Proprietary Core", ProductFamily = "Scanner API Wrapper", ProductName = "scanner-api" };
        try { OpenDataManager(); } catch { _isDsmOpen = false; }
    }
    ...
    private void OpenDataManager()
    {
        try
        {
            ushort rc = _is64Bit ? DSM64(ref _appIdentity, IntPtr.Zero, DG_CONTROL, 0x0301, MSG_OPENDSM, ref _appIdentity) : DSM32(ref _appIdentity, IntPtr.Zero, DG_CONTROL, 0x0301, MSG_OPENDSM, ref _appIdentity);
            if (rc == TWRC_SUCCESS) _isDsmOpen = true;
        }
        catch (DllNotFoundException) { Console.WriteLine($"[-] Missing system file mapping -> twain_{(_is64Bit ? "64" : "32")}.dll"); }
        catch { _isDsmOpen = false; }
    }
    ...
    public List<string> EnumerateScanners()
    {
        ...
        try
        {
            ...
            {
                if (!string.IsNullOrEmpty(sourceIdentity.ProductName)) scannerList.Add(sourceIdentity.ProductName);
                try { rc = _is64Bit ? DSM64(ref _appIdentity, IntPtr.Zero, DG_CONTROL, DAT_IDENTITY, MSG_GETNEXT, ref sourceIdentity) : DSM32(ref _appIdentity, IntPtr.Zero, DG_CONTROL, DAT_IDENTITY, MSG_GETNEXT, ref sourceIdentity); }
                catch { break; }
            }
        }
        catch (Exception ex) { Console.WriteLine($"[!] Critical data source query error: {ex.Message}"); }
        return scannerList;
    }
    ...
    public bool SelectAndOpenScanner(string scannerName)
    {
        ...
        try
        {
            ...
            while (rc == TWRC_SUCCESS)
            {
                ...
                rc = _is64Bit ? DSM64(ref _appIdentity, IntPtr.Zero, DG_CONTROL, DAT_IDENTITY, MSG_GETNEXT, ref sourceIdentity) : DSM32(ref _appIdentity, IntPtr.Zero, DG_CONTROL, DAT_IDENTITY, MSG_GETNEXT, ref sourceIdentity);
            }
        }
        catch { }
        return false;
    ...
    public bool ExecuteScanJob(int dpi, ushort pixelType, ushort paperSource)
    {
        ...
        try
        {
            try { SetCapability(0x1118, (uint)dpi); SetCapability(0x1119, (uint)dpi); } catch { }
            try { SetCapability(0x0101, pixelType); } catch { }
            try { SetCapability(0x1002, paperSource); } catch { }

            TW_USERINTERFACE ui = new TW_USERINTERFACE { ShowUI = 0, ModalUI = 0, ParentHand = IntPtr.Zero };
            ushort rc = _is64Bit ? DSM64Ui(ref _appIdentity, ref _activeSource, DG_CONTROL, DAT_USERINTERFACE, MSG_USERIFON, ref ui) : DSM32Ui(ref _appIdentity, ref _activeSource, DG_CONTROL, DAT_USERINTERFACE, MSG_USERIFON, ref ui);
    ...
    private void SetCapability(ushort capId, uint value)
    {
        ...
        try
        {
            ...
            if (hContainer == IntPtr.Zero) return;
            IntPtr pData = GlobalLock(hContainer);
            if (pData == IntPtr.Zero) return;

            TW_ONEVALUE oneVal = new TW_ONEVALUE { ItemType = 5, Item = value };
            Marshal.StructureToPtr(oneVal, pData, false);
            GlobalUnlock(hContainer);

            TW_CAPABILITY cap = new TW_CAPABILITY { Cap = capId, ConType = TWON_ONEVALUE, hContainer = hContainer };
            if (_is64Bit) DSM64Cap(ref _appIdentity, ref _activeSource, DG_CONTROL, DAT_CAPABILITY, 0x0002, ref cap);
            else DSM32Cap(ref _appIdentity, ref _activeSource, DG_CONTROL, DAT_CAPABILITY, 0x0002, ref cap);
        }
        finally { if (hContainer != IntPtr.Zero) GlobalFree(hContainer); }
    }
    ...
    public void CloseActiveScanner()
    {
        ...
        try
        {
            ushort rc = _is64Bit ? DSM64(ref _appIdentity, IntPtr.Zero, DG_CONTROL, DAT_IDENTITY, MSG_CLOSEDS, ref _activeSource) : DSM32(ref _appIdentity, IntPtr.Zero, DG_CONTROL, DAT_IDENTITY, MSG_CLOSEDS, ref _activeSource);
            if (rc == TWRC_SUCCESS) _isDsOpen = false;
        }
        catch { _isDsOpen = false; }
    }
